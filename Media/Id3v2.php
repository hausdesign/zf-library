...
/**#@+ @ignore */
require_once 'HausDesign/Media/Id3/Object.php';
require_once 'HausDesign/Media/Id3/Header.php';
/**#@-*/
 ...
 * @version    $Id: Id3v2.php 218 2011-05-02 19:35:18Z svollbehr $
 */
final class HausDesign_Media_Id3v2 extends HausDesign_Media_Id3_Object
{
    /** @var HausDesign_Media_Id3_Header */
    private $_header;

    /** @var HausDesign_Media_Id3_ExtendedHeader */
    ...
    /**
     * Constructs the HausDesign_Media_Id3v2 class with given file and options. The
     ...
     *     are limited to those supported by the {@link HausDesign_Media_Id3_Encoding}
     *     interface.
     ...
     * @throws HausDesign_Media_Id3_Exception if given file descriptor is not valid
     */
    public function __construct($filename = null, $options = array())
    {
        ...
        if ($filename === null) {
            $this->_header = new HausDesign_Media_Id3_Header(null, $options);
            return;
        }
        ...
        if ($this->_reader->read(3) != 'ID3') {
            require_once 'HausDesign/Media/Id3/Exception.php';
            throw new HausDesign_Media_Id3_Exception
                ('File does not contain ID3v2 tag');
        }

        $this->_header = new HausDesign_Media_Id3_Header($this->_reader, $options);
        ...
        if ($this->_header->getVersion() < 4 &&
            $this->_header->hasFlag(HausDesign_Media_Id3_Header::UNSYNCHRONISATION)) {
            $data = $this->_reader->read($this->_header->getSize());
            require_once 'HausDesign/Io/StringReader.php';
            $this->_reader = new HausDesign_Io_StringReader
                ($this->_decodeUnsynchronisation($data));
            $tagSize = $this->_reader->getSize();
        }
        $this->clearOption('unsynchronisation');
        if ($this->_header->hasFlag(HausDesign_Media_Id3_Header::UNSYNCHRONISATION)) {
            $this->setOption('unsynchronisation', true);
        }
        if ($this->_header->hasFlag(HausDesign_Media_Id3_Header::EXTENDED_HEADER)) {
            require_once 'HausDesign/Media/Id3/ExtendedHeader.php';
            $this->_extendedHeader =
                new HausDesign_Media_Id3_ExtendedHeader($this->_reader, $options);
        }
        if ($this->_header->hasFlag(HausDesign_Media_Id3_Header::FOOTER)) {
        ...
        while (true) {
            ...
            if (@fopen($file = 'HausDesign/Media/Id3/Frame/' .
                       ucfirst(strtolower($identifier)) . '.php', 'r',
                       true) !== false) {
                require_once($file);
            }
            if (class_exists
                ($classname = 'HausDesign_Media_Id3_Frame_' .
                     ucfirst(strtolower($identifier)))) {
                $frame = new $classname($this->_reader, $options);
            } else {
                require_once 'HausDesign/Media/Id3/Frame/Unknown.php';
                $frame =
                    new HausDesign_Media_Id3_Frame_Unknown($this->_reader, $options);
            }
     ...
     *
     * @return HausDesign_Media_Id3_Header
     */
    ...
    public function hasExtendedHeader()
    {
        if ($this->_header) {
            return $this->_header->hasFlag
                (HausDesign_Media_Id3_Header::EXTENDED_HEADER);
        }
        return false;
    }
     ...
     * otherwise.
     *
     * @return HausDesign_Media_Id3_ExtendedHeader|false
     */
     ...
     *
     * @param HausDesign_Media_Id3_ExtendedHeader $extendedHeader The header object
     */
    public function setExtendedHeader($extendedHeader)
    {
        if (is_subclass_of($extendedHeader, 'HausDesign_Media_Id3_ExtendedHeader')) {
            $this->_header->flags =
                $this->_header->flags | HausDesign_Media_Id3_Header::EXTENDED_HEADER;
            $this->_extendedHeader->setOptions($this->getOptions());
            $this->_extendedHeader = $extendedHeader;
        } else {
            require_once 'HausDesign/Media/Id3/Exception.php';
            throw new HausDesign_Media_Id3_Exception('Invalid argument');
        }
    }
     ...
     *
     * @param HausDesign_Media_Id3_Frame $frame The frame to add.
     * @return HausDesign_Media_Id3_Frame
     */
    ...
    {
        return $this->_header->hasFlag(HausDesign_Media_Id3_Header::FOOTER);
    }

    /**
     * Returns the footer object if present, or <var>false</var> otherwise.
     *
     * @return HausDesign_Media_Id3_Header|false
     */
    ...
    public function setFooter($useFooter)
    {
        if ($useFooter) {
            $this->_header->setFlags
                ($this->_header->getFlags() | HausDesign_Media_Id3_Header::FOOTER);
            $this->_footer = &$this->_header;
        } else {
    ...
    public function write($filename)
    {
        ...
        } else if ($filename !== null && $filename instanceof HausDesign_Io_Writer) {
    ...
    private function _writeData($writer)
    {
            ...
            if (($len = strlen($frameData)) != $frameDataLength) {
                ...
                $this->_header->setFlags
                    ($this->_header->getFlags() |
                     HausDesign_Media_Id3_Header::UNSYNCHRONISATION);
            } else {
                $this->_header->setFlags
                    ($this->_header->getFlags() &
                     ~HausDesign_Media_Id3_Header::UNSYNCHRONISATION);
        ...
        if ($this->hasExtendedHeader()) {
            ...
            if ($this->_extendedHeader->hasFlag
                (HausDesign_Media_Id3_ExtendedHeader::CRC32)) {
                $crc = crc32($frameData);
                if ($crc & 0x80000000) {
    ...
    public function __get($name)
    {
        ...
        if (@fopen($filename = 'HausDesign/Media/Id3/Frame/' . ucfirst($name) .
                   '.php', 'r', true) !== false) {
            require_once $filename;
        }
        if (class_exists
            ($classname = 'HausDesign_Media_Id3_Frame_' . ucfirst($name))) {
            return $this->addFrame(new $classname());
        }
        require_once 'HausDesign/Media/Id3/Exception.php';
        throw new HausDesign_Media_Id3_Exception('Unknown frame/field: ' . $name);
    }
