<?php
class HausDesign_View_Helper_FormTreeViewRadio extends Zend_View_Helper_FormElement
{
	...
	protected function _renderBranch($nodes)
	{
		$output = '<ul id="' . $this->_info['id'] . '"'.$this->_htmlAttribs($this->_info['attribs']) . '>';
			...
			foreach ($nodes as $node_id=>$node) {
				...
				$output .= '<li>';
					$output .= '<input ' . $checked . ' value="' . $node_id . '" id="' . $node_control_id . '" name="' . $node_control_name . '" type="radio" />';
	...
	public function formTreeViewRadio($name, $value = null, $attribs = null, $options = null)
	{
