<?php
class HausDesign_Controller_Plugin_Acl extends Zend_Controller_Plugin_Abstract
{
    /**
     ...
     * @var string
     */
    protected $_resourcePrefix = 'HausDesign_Controller_Plugin_Acl-';
     ...
     * Sets the ACL role.
     *
     * @param   array|Zend_Acl_Role_Interface $role   An ACL role.
     *
     * @return  HausDesign_Controller_Plugin_Acl
     */
    ...
    public function isActionAllowed($module, $controller, $action, $privilege = null)
    {
        ...
        */
        ...
        if ((! $acl->has($module)) && (! $acl->has($module)) && (! $acl->has($module))) {
            return true;
        } else {
            if (!$result && $acl->has($module)) {
                if ($this->isAllowed($module, $privilege)) {
                       $result = true;
                } else {
                    $result = false;
                }
            ...
            if (!$result && $acl->has($controller)) {
                if ($this->isAllowed($controller, $privilege)) {
                    $result = true;
                } else {
                    $result = false;
                }
            ...
            if (!$result && $acl->has($action)) {
                if ($this->isAllowed($action, $privilege)) {
                    $result = true;
                } else {
                    $result = false;
                }
