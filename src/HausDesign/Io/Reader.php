...
/**
 * The HausDesign_Io_Reader class represents a character stream providing means to
...
class HausDesign_Io_Reader
{
    ...
    /**
     * Constructs the HausDesign_Io_Reader class with given open file descriptor.
     *
     * @param resource $fd The file descriptor.
     * @throws HausDesign_Io_Exception if given file descriptor is not valid
     */
    ...
    public function getOffset()
    {
        if ($this->_fd === null) {
            require_once('HausDesign/Io/Exception.php');
            throw new HausDesign_Io_Exception('Cannot operate on a closed stream');
        }
     ...
     * @return void
     * @throws HausDesign_Io_Exception if <var>size</var> attribute is negative or if
     *  an I/O error occurs
     */
    public function skip($size)
    {
        if ($size < 0) {
            require_once('HausDesign/Io/Exception.php');
            throw new HausDesign_Io_Exception('Size cannot be negative');
        }
        if ($size == 0) {
            return;
        }
     ...
     * @return string
     * @throws HausDesign_Io_Exception if <var>length</var> attribute is negative or
    ...
    public function read($length)
    {
        if ($length < 0) {
            require_once('HausDesign/Io/Exception.php');
            throw new HausDesign_Io_Exception('Length cannot be negative');
        }
        if ($length == 0) {
            return '';
        }
    ...
    public function __get($name)
    {
        if (method_exists($this, 'get' . ucfirst(strtolower($name)))) {
            ...
            require_once('HausDesign/Io/Exception.php');
            throw new HausDesign_Io_Exception('Unknown field: ' . $name);
        }
    }
